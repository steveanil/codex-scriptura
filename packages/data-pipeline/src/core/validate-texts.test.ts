...
import { validateTranslation, alignmentProblems, CANONICAL_CHAPTERS, KNOWN_TITLE_LEMMA_LEAKS, type ValidationVerse } from './validate-texts.js';
...
describe('validateTranslation - warnings vs expected omissions', () => {
    it('fails on an unexpected verse gap - a dropped verse is a hard error (issue #177 class)', () => {
        ...
        const verses = Array.from({ length: 13 }, (_, i) => v('2John', 1, i + 1))
            .filter((x) => x.verse !== 2);
        const r = validateTranslation('T', verses);
        expect(r.errors).toEqual(['T: verse gap at 2John.1.2']);
        expect(r.warnings).toEqual([]);
...
describe('validateTranslation - trailing-verse check (known-issues #24)', () => {
    ...
    it('warns when a chapter runs past the reference count', () => {
        ...
        expect(r.checks.map((c) => `${c.id}:${c.severity}`)).toEqual(['missing-chapters:warning', 'verse-overrun:warning']);
    });
...
describe('detached punctuation (issue #175)', () => {
    ...
    it.each([
        ...
        expect(validateTranslation('T', withVerseOne({ text })).errors).toEqual([]);
    });
...
describe('Psalm superscriptions in verse 1 (issue #176 class)', () => {
    ...
    it.each(['To the chief Musician on Neginoth, A Psalm of David. Hear me', 'A Psalm of David. Jehovah, who', 'For the choirmaster. With stringed instruments.'])(
        'fails on title text opening verse 1: %s', (text) => {
            ...
            expect(r.errors).toEqual([`ASV: superscription text in Ps.4.1: "${text.slice(0, 40)}"`]);
            expect(r.checks.filter((c) => c.severity === 'error')).toEqual([{ id: 'psalm-title-text', severity: 'error', count: 1 }]);
        });

    it('lets the YLT keep titles in verse 1, where its source numbers them', () => {
        expect(validateTranslation('YLT', ps4({ text: 'A Psalm of David. Jehovah, who' })).errors).toEqual([]);
    });

    it('does not mistake a body verse that mentions a psalm for a title', () => {
        expect(validateTranslation('ASV', ps4({ text: 'Sing unto him a new song; a psalm of praise' })).errors).toEqual([]);
    });
    ...
    it('fails on superscription lemmas in verse 1 (mizmor, lamnatseach, michtam, maskil, shiggaion)', () => {
        for (const token of ['H4210', 'H5329', 'H4387', 'H4905', 'H7692']) {
            ...
            expect(r.errors).toEqual([`WEB: superscription lemma(s) in Ps.4.1: ${token}`]);
        }
    });

    it('accepts the same tokens outside verse 1 and outside Psalms', () => {
        expect(validateTranslation('WEB', ps4({}).map((x) => (x.verse === 2 ? { ...x, lemmas: 'H4210' } : x))).errors).toEqual([]);
        expect(validateTranslation('WEB', withVerseOne({ lemmas: 'H4210' })).errors).toEqual([]);
    });

    it('excuses eBible\'s source-tagged verse-1 records for ASV and BSB only', () => {
        expect(KNOWN_TITLE_LEMMA_LEAKS.ASV.has('Ps.4.1')).toBe(true);
        expect(validateTranslation('ASV', ps4({ lemmas: 'H430 H4210' })).errors).toEqual([]);
        expect(validateTranslation('DBY', ps4({ lemmas: 'H430 H4210' })).errors).toHaveLength(1);
        ...
        expect(validateTranslation('ASV', ps5).errors).toHaveLength(1);
    });
});
...
describe('lemma tokens', () => {
    it.each(['strong:H7225', 'H07225', 'H0', 'h7225', 'H123456', 'G25a'])('fails on the unnormalized token %s', (token) => {
        ...
        expect(r.errors).toEqual([`T: malformed lemma token(s) in 2John.1.1: ${token}`]);
    });

    it('accepts normalized Hebrew and Greek tokens', () => {
        expect(validateTranslation('T', withVerseOne({ lemmas: 'H1 H7225 G25 G99999' })).errors).toEqual([]);
    });
});
...
describe('alignment spans', () => {
    ...
    it('accepts well-formed, ordered spans whose tokens are in the lemma bag', () => {
        ...
        expect(validateTranslation('T', withVerseOne(good)).errors).toEqual([]);
    });
