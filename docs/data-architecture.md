...
The descriptor the manifest ships is built, not written: `describeResource` takes the title, author, publisher and description from the definition and everything about license and provenance from the source registry and the accepted checksums, so the credits screen cannot say something the pipeline did not fetch. The resource's license is its primary source's unless the definition names the one that governs a mixed derivative, which is always the most demanding source's: cross-references are CC BY-SA 4.0 because the UBS overlay is, and the WEB is CC BY 4.0 because its Strong's tagging is derived from OSHB lemma data even though the text itself is public domain. `describeResource` refuses a resource that claims the public domain over a source that asks for credit. Every source keeps its own license and, where the upstream asks for it, its attribution wording in `provenance`; a source under a permissive software license (BSD, MIT, Apache) carries instead the notice redistributions must reproduce, verbatim from its LICENSE at the pinned commit (`licenseNotice`). Both live on the `SourceDataset` in the source registry, never in a resource definition, and tests refuse a CC BY source without attribution or a BSD/MIT/Apache source without a notice. A translation's catalog record (`TranslationMeta`) is derived from its resource the same way, so the picker and the credits screen cannot disagree. Adding a license means adding a row to `LICENSES` there, and the tests fail until it exists.
...
| 2 | OT-NT-Reference-Map / UBS Parallel Passages | Type classification overlay - the only source of `quotation`. UBS verse pairs are typed from their per-word match data (quotation when at least half the NT verse fully matches, or a five-word run does); OT-NT chapter pairs carry curated q/a/p codes, and a chapter-level quotation needs the verse pair attested at 3 votes. The higher-priority label wins when both match; see `parse-typed-overlays.ts` |
