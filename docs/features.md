...
- **Reader** - translation (the reader's active one; switching in the reader updates it too), open at launch (last read or a fixed passage, book list follows the active translation's canon), column width, line spacing, layout density, verse numbers, reading speed (for time estimates), prose vs verse-per-line (Prose is the default; split view is always verse-per-line), red letter (WEB), cross-reference markers, divergence shading, synced scrolling (the last three mirror the reader's split-view toggles).
