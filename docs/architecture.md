...
2. **Seeding is gated per dataset by manifest comparison** (`getDatasetState(entry)` against the `datasets` row), not a global flag or a row count. `current` skips; `missing`, `stale` and `legacy` install.
...
5. **Installed state is reactive.** `datasetStatus` (`src/lib/stores/datasetStatus.svelte.ts`) combines a Dexie live query on the `datasets` table with the boot plan and in-flight progress, so `state(id)` answers `installed`, `loading`, `failed` or `absent` for any dataset and flips the moment an identity row lands. The first-run screen lists every dataset this boot installs with its size and progress; after the reader is live a neutral strip above the content shows what is still coming. Failures surface through `seedStatus` as before.
