...
`packages/db` is split by domain (issue #320, decision D9). `database.ts` defines `CodexDB` (database name `codex-scriptura`) and the one instance; `schema.ts` holds every table definition and migration, currently at **schema version 34**; `index.ts` is a barrel and the app imports everything from `@codex-scriptura/db`. Query modules follow the tables: `verses.ts`, `translations.ts`, `preferences.ts` (settings and kv), `annotations.ts` (annotations, themes, tags), `saved-searches.ts`, `word-search.ts`, `entities.ts`, `search-indexes.ts`, `cross-references.ts`, `topics.ts`, `lexicon.ts`, `strongs.ts`, `relationships.ts`, `datasets.ts`, `aggregates.ts`, `resources.ts` and `commentary.ts`, each with its test beside it. There is deliberately no repository interface or injection layer: the package is already the data-access boundary. Tables:
...
1. Best-effort `navigator.storage.persist()`, load preferences (the active translation decides what is boot-critical), then `seedCritical()`: fetch `/data/manifest.json`, upsert the translation catalog, install the active or default translation. `ready` flips, the What's New check runs, and `seedEnhancements()` continues without being awaited: the other wanted translations, cross-references, the Theographic entities, genealogy, the lexicon, the topical index and then every commentary dataset the manifest lists (found by resource type, not by name; issue #83), sequentially. Any throw before `ready` renders a retry screen instead of an infinite spinner; a failure after it is a banner.
...
Three modes on `/search`, plus the Cmd+K command palette. The route (`src/routes/search/+page.svelte`) is a shell: it owns the query, the mode, the translation and testament filters, saved searches and deep links, and composes one component per mode from `src/lib/components/search/` (`FullTextResults`, `WordStudyResults`, `TopicsResults`), each owning its own results and search call. A mode runs its search when the shell bumps a `seq` prop (typing settled, a filter changed, a saved search applied) or once when it mounts after a mode switch; Full Text also re-runs when an index becomes ready, Word Study and Topics when a dataset they answer from lands. The shell's view of which selected translations have an index ready, building or failed is `SearchIndexSet` (`src/lib/search/index-set.svelte.ts`), which asks the index manager and releases what the page stops selecting; no index or db code lives in the route.


- **Full text** - MiniSearch per translation (queries the `text` + `lemmas` fields, prefix and length-dependent fuzzy matching, stop words, exact-phrase re-ranking, testament filter, merged across selected translations). `src/lib/search/index-manager.ts` is the only code that builds, caches, loads and releases an index (`getOrBuildIndex`); the page and the command palette share it. An index stores ids only and hits are hydrated from `verses` with `bulkGet`. A cached copy is current while its key matches the translation's installed dataset version and hash plus the index configuration (`INDEX_CONFIG_KEY` in `src/lib/search/config.ts`), so a corpus refresh or a config change rebuilds it without a schema migration. Deselecting a translation or leaving the page frees its in-memory index.
- **Word Study** (concordance) - exhaustive, but it reads only candidate verses (issue #166); no search path loads a whole translation. The shared index carries a third field, `stems`: the verse text again, every term reduced by the Word Study stemmer, stop words kept (a concordance of "unto" is legitimate). Raw terms and stems never share a field, because the stemmer maps "made" to "mad". `src/lib/search/concordance.ts` enumerates the spellings the query allows (`wordForms` in `config.ts`, the same list the whole-word regex is built from), stems each, looks those stems up exactly (no prefix, no fuzzy), filters the ids by testament, and passes them to `wordSearch` / `lemmaGroupSearch` in `packages/db`, which `bulkGet` the candidates and let the regex decide what is an occurrence, its surface and its count. `lemmaGroupSearch` uses the `align` spans to group English surface forms by underlying Strong's lemma (gated on `Translation.aligned`). Lexicon entries (`searchLexicon`, diacritic-folded) render alongside.
- **Strong's** (`H7225`-style queries) - `strongsSearch` reads one `strongsPostings` row and `bulkGet`s the verses it lists, testament filter applied to the ids first. Each tagged translation has its own postings dataset, `strongs-index:<id>`, derived from its verses file. It is acceleration data with no authority: `translationInstallPlan` and `removeTranslationData` delete a translation's postings and their receipt in the same transaction that clears its verses, so new verses are never searched through old postings; the matching postings install afterwards inside the translation's lock (`seedStrongsIndexLocked`), only when the installed translation's content hash is the one the manifest says they were built from, and never for catalog-only translations. `strongsSearch` answers only while the postings' receipt stands, so an interrupted install reads as "not installed yet", not as partial results.
