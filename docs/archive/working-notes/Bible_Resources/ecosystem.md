...
### Phase 2: v0.6 → v1.0 (The Plugin Synthesis)
*   **Action for Blog:** Export all existing blog posts covering specific verses into a structured format.
*   **Action for App:** Build the first Extensibility Sandbox (`v0.6.0`). Use your exported blog data as the guinea pig to build the "Commentary Pane" plugin type.

### Phase 3: v2.0+ (The Seamless Web)
*   **Action for Blog:** Implement "Hover to read" JavaScript on the blog that pings the hosted version of Codex Scriptura (`app.bibleapologist.com` or `codexscriptura.org`) to fetch verse popups via API.
*   **Action for App:** Integrate the AI Study Assistant, leveraging `bibleapologist.com` as a highly-weighted, trusted constraint for RAG (Retrieval-Augmented Generation) answers regarding theological objections.
