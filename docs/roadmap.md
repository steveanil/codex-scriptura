...
*Status: In progress — graph view, genealogy viewer, split view, verse hover previews, quotation badges, and all data imports are complete. Remaining: Strong's search (data acquisition), lexicon lookup UI, graph enrichment, theme threading, scratch pad, semantic search, and maps.*
...
- [ ] **Pipeline:** Source checksum pinning (see open-data-sources.md → Accuracy & Reproducibility Hardening). Per-chapter canonical verse-count table to catch trailing-verse omissions (documented limitation of gap analysis).
