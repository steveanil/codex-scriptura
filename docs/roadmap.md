...
*Status: In progress - graph view, genealogy viewer, split view, verse hover previews, quotation badges, and all data imports are complete. Remaining: Strong's search (data acquisition), lexicon lookup UI, graph enrichment, theme threading, scratch pad, semantic search, and maps. **Unreleased:** all completed items here are merged to `main` but untagged; the latest release is still `v0.3.2` (see Release Status above).*
...
- [x] **Interactive Genealogy Viewer:** Visualize person relationships from the `relationships` Dexie table using `buildPersonSubgraph(personId, depth)` in `src/lib/engines/genealogy.ts`.
...
*Full findings with file/line detail live in [known-issues.md](known-issues.md). Versioning note: the checked items below are on the `fix/known-issues-hardening` branch. Because this work is landing before the v0.4.0 milestone finishes, it will ship inside the `v0.4.0` release if merged first; "v0.4.1" is this section's planning label, not a promise of a separate tag.*
...
- [ ] **Pipeline:** Source checksum pinning (see open-data-sources.md → Accuracy & Reproducibility Hardening). ~~Per-chapter canonical verse-count table to catch trailing-verse omissions~~ - done 2026-07-18 (`kjv-versification.ts` generated from the raw KJV source; trailing check in `validate-texts.ts`).
