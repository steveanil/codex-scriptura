...
- [x] **Search: Strong's Number Search** (e.g. H430 → Elohim) - **shipped 2026-07-18.** The eBible.org expansion translations (ASV, BSB, DBY) ship word-level Strong's markup (`<w s="H7225">`) which the USFX importer harvests into `VerseRecord.lemmas` (~31K tagged verses per translation, both testaments), and the wiring landed the same day: a Strong's-shaped Word Study query (H7225, g26, G0026) runs `strongsSearch()` against lemma tokens, shows the lexicon entry above the results, and falls back to all tagged translations with an explanatory note when the user's selected translations aren't tagged. `Translation.strongs` marks tagged translations; `getStrongsForVerse()` is implemented for future verse-level UI. Caveats: lemma tags are verse-level (no word alignment, so no in-verse highlighting), and upstream tagging is uneven - ASV/BSB lack whole Greek ranges (e.g. G25/G26) that DBY carries, which is why the fallback searches every tagged translation. KJV/WEB remain untagged; STEPBible TAHOT/TAGNT or the CrossWire KJV module (see [open-data-sources.md](open-data-sources.md#strongs-tagged-sources-unblocks-v040-strongs-search)) can tag the KJV later.
...
- [x] **Lexicon lookups (basic):** the `/search` Lexicon tab surfaces Strong's numbers, lemmas, transliterations, glosses, and expandable definitions from the seeded `lexicon` table (searchable by ID, lemma, transliteration, or English gloss). **Tap-through landed 2026-07-18 with Strong's number search:** every expanded entry carries a "See every occurrence" button that jumps to the Word Study concordance for that Strong's number, and the "preview" badge came off.
...
- [ ] **Search evolution: fold Lexicon into Word Study** *(decided 2026-07-18; partially arrived the same day)*. The first bridge landed with Strong's number search: a Strong's ID typed into Word Study shows its lexicon entry above the concordance results, and lexicon entries link to their occurrences. What remains is the English-query half: a query like "love" grouping results by underlying lemma (agapao 143x, phileo 25x), each lemma expanding to every verse containing that Strong's number regardless of English rendering (chesed → mercy, kindness, lovingkindness). That grouping needs word-aligned (not verse-level) tags to map an English surface word to *its* lemma rather than to every lemma in the verse. At that point the Lexicon tab stops being a separate destination: its entry cards become the group headers inside Word Study results and the standalone tab is retired (or kept as a thin alias). This is the Strong's Exhaustive Concordance workflow, not an interlinear; the interlinear is a reader feature and needs the word-aligned data listed under v0.5.0.
