                    ...
                    {#each verses as verse}
                        ...
                        >
                            ...
                            <!-- The badges are plain inline spans (role=button) with the
                                 ...
                                 reason. Click opens the row; there is no hover behaviour. -->
                            {@html buildVerseHtml(verse, getEntitiesForVerse(verse), parseWjRanges(verse.wj))}{#if showRefs && (refCount > 0 || quotationRefs.length > 0)}<span class="verse-badges">{'\u2060'}{#if refCount > 0}<span
                                    ...
                                    aria-label="{refCount} cross-reference{refCount === 1 ? '' : 's'} for verse {verse.verse}"
                                ...
                                ><span class="xref-icon"></span><span class="xref-count">{refCount}</span></span>{/if}{#if quotationRefs.length > 0}<span
                                    ...
                                    aria-label="Verse {verse.verse} quotes earlier scripture ({quotationRefs.length} source{quotationRefs.length === 1 ? '' : 's'})"
                                    title="Quotes earlier scripture ({quotationRefs.length} source{quotationRefs.length === 1 ? '' : 's'})"
...
<style>
    ...
    /* Touch: a taller tap target without changing the inline layout */
